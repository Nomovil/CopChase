window.addEventListener("message", (event) => {
    ...
    const visible = data.slowed || data.speedboost || data.invCtrl || data.wanted || data.ramp || data.fixcar || data.slowcops || data.slowthief;
...
function modMessage(elementName, text, visible, alpha, color = "white", percent = false) {
    const new_alpha = mapValue(alpha, 0, 1, 0.5, 1);
    if (alpha <= 0.25) {
